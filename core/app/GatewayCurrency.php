...
class GatewayCurrency extends Model
{
    ...
    {
        return ($this->image) ? asset(config('constants.deposit.gateway.path') . '/' . $this->image) : (($this->method->image) ? asset(config('constants.deposit.gateway.path') . '/' . $this->method->image) :  asset(config('constants.image.default')));
    }
}
