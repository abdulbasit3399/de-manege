...
class SupportTicketController extends Controller
{
  public function supportTicket()
  {
    ...
    $items = SupportTicket::latest()->with('user')->paginate(config('constants.table.default'));
  ...
  public function pendingSupportTicket()
  {
    ...
    $empty_message = 'No Data found.';
    $items = SupportTicket::whereIN('status', [0, 2])->latest()->with('user')->paginate(config('constants.table.default'));
    return view('admin.support.tickets', compact('items', 'page_title', 'empty_message'));
  }


  public function answerSupportTicket()
  {
    $page_title = 'Answered Tickets';
    $empty_message = 'No Data found.';
    $items = SupportTicket::latest()->with('user')->whereIN('status', [1])->paginate(config('constants.table.default'));
    return view('admin.support.tickets', compact('items', 'page_title', 'empty_message'));
  }
  ...
  public function closedSupportTicket()
  {
    ...
    $items = SupportTicket::whereIN('status', [3])->latest()->with('user')->paginate(config('constants.table.default'));
    return view('admin.support.tickets', compact('items', 'page_title', 'empty_message'));
  }
