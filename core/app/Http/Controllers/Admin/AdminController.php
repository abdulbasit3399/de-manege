...
class AdminController extends Controller
{
    ...
    public function dashboard(Request $request)
    {
        ...
        $widget['user_balance'] = UserWallet::sum('balance');
    ...
    public function profileUpdate(Request $request)
    {
        ...
        if ($request->hasFile('image')) {
            try {
                $old = $user->image ?: null;
                $user->image = upload_image($request->image, config('constants.admin.profile.path'), config('constants.admin.profile.size'), $old);
            } catch (\Exception $exp) {
